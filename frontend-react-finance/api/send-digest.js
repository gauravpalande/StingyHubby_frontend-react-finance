...
export default async function handler(req, res) {
  try {
    ...
    if (prefError) {
      console.error('[send-digest] prefError', prefError);
      return res.status(500).json({ error: prefError.message });
    }
    if (!prefs?.length) {
      console.log('[send-digest] no opted-in users');
    ...
    if (userError) {
      console.error('[send-digest] userError', userError);
      return res.status(500).json({ error: userError.message });
    }
    if (!users?.length) {
      console.log('[send-digest] no users matched ids');
      return res.status(200).json({ message: 'No users matched ids' });
    }
    ...
    for (const user of users) {
      try {
        ...
        if (historyError) {
          console.warn('[send-digest] historyError', { userId: user.id, error: historyError.message });
          continue;
        }
        if (!history?.length) {
          console.log('[send-digest] no history', { userId: user.id });
          continue;
        }
        ...
        try {
          ...
          console.warn('[send-digest] chart fetch failed', e?.message || e);
        ...
        if (user.paid_user) {
          ...
          try {
            ...
            console.warn('[send-digest] pdf build failed', e?.message || e);
          }
        }
        ...
        console.error('[send-digest] per-user failure', { userId: user?.id, error: e?.message || e });
      }
    }

    console.log('[send-digest] done', { sentCount });
